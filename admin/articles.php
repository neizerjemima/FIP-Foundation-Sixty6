...
$stmt = $connection->prepare('SELECT v.*, r.first_name AS author_fname, r.last_name AS author_lname FROM articles v JOIN authors r ON v.author = r.id ORDER BY v.created_at ASC');
$stmt->execute();
?>
...
<body class="user-website" data-page="volunteer-principal-cms">
    ...
    <main class="content-user">
        ...
        <section class="grid-con">
            <h2 class="hidden">Articles Table</h2>

            <div class="col-start-2 col-end-5 m-col-start-6 m-col-end-13 xl-col-start-7 xl-col-end-13 buttons-more-info">
                <a href="article_add_form.php"><button class="article-button-user">CREATE</button></a>
                <a href="authors.php"><button class="article-button-user">AUTHORS</button></a>
            </div>
                    ...
                    <table>
                        <thead>
                                <tr class="column-names">
                                    <td>Title</td>
                                    <td>Image</td>
                                    <td class="desktop-options hidden">Author</td>
                                    <td class="desktop-options hidden">Headline</td>
                                    <td class="desktop-options hidden">Text</td>
                                    <td>Action</td>
                                </tr>
                                <tbody>
                                    <tr>
                                ...
                                <?php
                                 ...
                                 while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                    $first_three_words_text = implode(' ', array_slice(explode(' ', $row['text']), 0, 3));
                                    $first_three_words_phrase = implode(' ', array_slice(explode(' ', $row['attention_phrase']), 0, 3));
                                    echo 
                                        '<td>'.$row['title'].'</td>
                                        <td>'.$row['image'].'</td>
                                        <td class="email-volunteer hidden">'.$row['author_fname'].' '.$row['author_lname'].'</td>
                                        <td class="email-volunteer hidden">'.$first_three_words_phrase.'...</td>
                                        <td class="notes-volunteer hidden">'.$first_three_words_text.'...</td>
                                        <td class="buttons-column">
                                            <a href="article_more.php?id='.$row['id'].'"><button class="button-table-general">More</button></a>
                                            <a href="article_edit.php?id='.$row['id'].'"><button class="button-table-general">Edit</button></a>
                                            <a href="delete_article.php?id='.$row['id'].'"><button class="button-table-general">Delete</button></a>
                                        </td>
                                    </tr>';
                                 }
