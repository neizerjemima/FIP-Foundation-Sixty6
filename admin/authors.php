...
$stmt = $connection->prepare('SELECT * FROM authors ORDER BY created_at ASC');
$stmt->execute();
?>
                        ...
                        <thead>
                                ...
                                <?php
                                 ...
                                 while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                    $first_three_words = implode(' ', array_slice(explode(' ', $row['about']), 0, 3));
                                    echo 
                                        '<td>'.$row['first_name'].'</td>
                                        <td>'.$row['last_name'].'</td>
                                        <td class="notes-volunteer hidden">'.$first_three_words.'...</td>
                                        <td class="buttons-column">
                                            <a href="author_more.php?id='.$row['id'].'"><button class="button-table-general">More</button></a>
                                            <a href="author_edit.php?id='.$row['id'].'"><button class="button-table-general">Edit</button></a>
                                            <a href="delete_author.php?id='.$row['id'].'"><button class="button-table-general">Delete</button></a>
                                        </td>
                                    </tr>';
                                 }
