...
$stmt = $connection->prepare('SELECT * FROM contacts ORDER BY created_at ASC');
$stmt->execute();
?>
...
<body class="user-website" data-page="volunteer-principal-cms">
    ...
    <main class="content-user">
        ...
        <section class="grid-con">
            <h2 class="hidden">Messages Table</h2>
                        ...
                        <thead>
                                ...
                                <?php
                                 ...
                                 while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                      $first_three_words = implode(' ', array_slice(explode(' ', $row['textinput']), 0, 3));
                                    echo 
                                        '<tr><td>'.$row['name'].'</td>
                                        <td>'.$row['subject'].'</td>
                                        ...
                                        <td class="buttons-column">
                                          <a href="message_more.php?id='.$row['id'].'"><button class="button-table-general">More</button></a>
                                            <a href="delete_message.php?id='.$row['id'].'"><button class="button-table-general">Delete</button></a>
                                        </td>
                                    </tr>';
                                 }
